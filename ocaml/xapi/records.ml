...
let pool_record rpc session_id pool =
  ...
  { setref=(fun r -> _ref := r; record := empty_record );
    ...
    fields = [
      ...
      make_field ~name:"nbd-insecure" ~get:(fun () -> string_of_bool (x ()).API.pool_nbd_insecure) ~set:(fun s -> Client.Pool.set_nbd_insecure rpc session_id pool (safe_bool_of_string "nbd-insecure" s)) ();
    ]}
